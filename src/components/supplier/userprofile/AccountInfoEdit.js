...
const AccountInfoEdit = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const url = "sleeping-owl-we0m.onrender.com/api/supplier/profile/bank-account/change-request";
    ...
    const handleImageDelete = async (index, type, id) => {
        ...
        try {
            ...
            const url = `sleeping-owl-we0m.onrender.com/api/supplier/${formData.id}/bank-account/image/${index}?type=${type}`;
            const response = await fetch(url, {
                method: "DELETE",
                headers: {
                    Authorization: `Bearer ${token}`,
