...
const BusinessInfo = () => {
  ...
  const fetchState = useCallback(async (id) => {
    ...
    try {
      ...
      const response = await fetch(
        `sleeping-owl-we0m.onrender.com/api/location/country/${id}/states`,
        {
          method: "GET",
          headers: {
  ...
  const fetchCity = useCallback(async (id) => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`sleeping-owl-we0m.onrender.com/api/location/state/${id}/cities`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${suppliertoken}`,
  ...
  const handleImageDelete = async (index, type) => {
    ...
    try {
      ...
      const url = `sleeping-owl-we0m.onrender.com/api/supplier/${formData.id}/company/${formData.companyid}/image/${index}?type=${type}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = `sleeping-owl-we0m.onrender.com/api/supplier/profile/update`; // Ensure the URL is correct
