  ...
  const handleFileChange = (event, index) => {
   ...
   const handleImageDelete = async (index,type,id) => {
              ...
              try {
                  ...
                  const url = `http://https://sleeping-owl-we0m.onrender.com/api/supplier/${formData.id}/bank-account/${id}/image/${index}?type=${type}`;
  ...
  const handleSubmit = async (e) => {
    if (validate()) {
      ...
      try {
        ...
        const url = `http://https://sleeping-owl-we0m.onrender.com/api/supplier/profile/update`; // Ensure the URL is correct
