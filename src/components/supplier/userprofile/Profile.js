...
export default function Profile() {
  ...
  const fetchSupplier = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`http://https://sleeping-owl-we0m.onrender.com/api/supplier/profile`, {
        method: "GET",
        headers: {
    ...
    const fetchCity = useCallback(async (id) => {
          ...
          try {
              setLoading(true);
              const response = await fetch(`http://https://sleeping-owl-we0m.onrender.com/api/location/state/${formData?.permanentState||id}/cities`, {
                  method: "GET",
                  headers: {
                      "Content-Type": "application/json",
                      Authorization: `Bearer ${suppliertoken}`,
      ...
      const fetchState = useCallback(async (id) => {
          ...
          try {
            ...
            const response = await fetch(
              `http://https://sleeping-owl-we0m.onrender.com/api/location/country/${ formData?.permanentCountry|| id}/states`,
              {
                method: "GET",
                headers: {
                  "Content-Type": "application/json",
