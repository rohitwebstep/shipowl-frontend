...
const ProfileEditProvider = ({ children }) => {
    ...
    const fetchCountry = useCallback(async () => {
        ...
        try {
            setLoading(true);
            const response = await fetch(`http://https://sleeping-owl-we0m.onrender.com/api/location/country`, {
                method: "GET",
                headers: {
  ...
  return (
    <ProfileEditContext.Provider value={{businessErrors, setBusinessErrors,errors,setErrors,validateBusiness,validate,requiredFields,fetchCountry,files, setFiles,cityData,setCityData,stateData,setStateData, formData,activeTab,countryData,setActiveTab, setFormData }}>
      {children}
