...
const AccountInfo = () => {
  ...
  const handleImageDelete = async (index, type, id) => {
    ...
    try {
      ...
      const url = `sleeping-owl-we0m.onrender.com/api/supplier/${formData.id}/company/${id}/image/${index}?type=${type}`;
  ...
  const handleSubmit = async (e) => {
    if (validate()) {
      ...
      try {
        ...
        const url = `sleeping-owl-we0m.onrender.com/api/supplier/${id}`; // Ensure the URL is correct
