...
const AccountInfo = () => {
  ...
  const handleImageDelete = async (index, type, id) => {
    ...
    try {
      ...
      const url = `https://shipping-owl-vd4s.vercel.app/api/supplier/${formData.id}/company/${id}/image/${index}?type=${type}`;
  ...
  const handleSubmit = async (e) => {
    if (validate()) {
      ...
      try {
        ...
        const url = `https://shipping-owl-vd4s.vercel.app/api/supplier/${id}`; // Ensure the URL is correct
