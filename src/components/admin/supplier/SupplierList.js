...
const SupplierList = () => {
    ...
    const fetchSupplierStatus = useCallback(async (id, item) => {
        ...
        try {
            setLoading(true);
            const response = await fetch(`https://shipping-owl-vd4s.vercel.app/api/admin/supplier/${id}/status?status=${item}`, {
                method: "PATCH",
                headers: {
                                ...
                                return (
                                    <React.Fragment key={item.id}>
                                        <tr className="bg-transparent border-b border-[#E9EDF7] text-[#2B3674] font-semibold">
                                            ...
                                            <td className="p-3 px-4 text-left">{item.currentAddress || '-'}</td>
