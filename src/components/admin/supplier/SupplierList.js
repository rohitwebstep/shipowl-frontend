...
const filteredSuppliers = suppliers.filter((supplier) => {
    ...
    const fetchSupplierStatus = useCallback(async (id, item) => {
        ...
        try {
            setLoading(true);
            const response = await fetch(`https://sleeping-owl-we0m.onrender.com/api/supplier/${id}/status?status=${item}`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${admintoken}`,
                        ...
                        <table className="display main-tables w-full" id="supplierTable">
                            <thead>
                                <tr className="border-b text-sm font-semibold text-[#A3AED0]">
                                    ...
                                    <th className="p-3 px-4 text-left uppercase whitespace-nowrap">Date Of Birth</th>
                                            ...
                                            <tr className="border-b text-[#2B3674] font-semibold">
                                                ...
                                                <td className="p-3 px-4 text-left whitespace-nowrap">
                                                    <span
                                                        ...
                                                        className={`p-2 cursor-pointer rounded-md text-sm text-left whitespace-nowrap text-white ${item.status?.toLowerCase() === 'inactive'
                                                            ? 'bg-green-500'
