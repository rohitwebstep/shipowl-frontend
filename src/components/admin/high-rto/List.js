...
export default function List() {
    ...
    const fetchCity = useCallback(async () => {
        try {
            const response = await fetch("sleeping-owl-we0m.onrender.com/api/location/city");
    ...
    const fetchState = useCallback(async () => {
        try {
            const response = await fetch("sleeping-owl-we0m.onrender.com/api/location/state");
            const result = await response.json();
