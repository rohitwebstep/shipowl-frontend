...
const Payment = () => {
  ...
  const handleImageDelete = async (index, type) => {
    ...
    try {
      ...
      const url = `sleeping-owl-we0m.onrender.com/api/admin/${formData.id}/company/${formData.companyid}/image/${index}?type=${type}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = `sleeping-owl-we0m.onrender.com/api/admin/dropshipper/${formData?.id}/profile/update`;
